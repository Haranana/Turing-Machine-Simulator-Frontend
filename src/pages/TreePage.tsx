...
export default function TreePage(){
  ...
  const [graphData, setGraphData ] = useState<GraphData>({children: [], edges: []});
  ...
  useEffect(() => {
    ...
    const getLayout = async () => {
      ...
      const elkEdges: Edge[] = res.edges?.map((edge) => ({
        ...
        type: "straight",
        className: isEdgeSelected(Number(edge.id))? "SimulationEdge SelectedNode" : "SimulationEdge",
      })) ?? [];
  ...
  //creates path based on chosen transition
    ...
    const onTransitionChosen = (nodesMapChildId: number) => {
      ...
      graphData.children.forEach((node)=>{
        if(nodeInPath(node.id)) nodes.set(node.id, true);
        else nodes.set(node.id, false);
      });
